...
public class day7 {
    ...
    public static int exerciseOne(boolean one) throws FileNotFoundException {
        ...
        for (int i = 1; i < 2000; i++) {
            int fuel = 0;
            for (Entry<Integer, Integer> crab : crabs.entrySet()) fuel += one ? Math.abs(crab.getKey() - i) * crab.getValue() : Math.abs(crab.getKey() - i) * (Math.abs(crab.getKey() - i) + 1) / 2 * crab.getValue();
